...
const ResultsPage = () => {
  ...
  const [explanations, setExplanations] = useState<string[]>(Array(mockQuestions.length).fill(''));
  const [loading, setLoading] = useState<boolean[]>(Array(mockQuestions.length).fill(false));
  const [error, setError] = useState<string[]>(Array(mockQuestions.length).fill(''));
  ...
  const getExplanation = async (questionIndex: number) => {
    ...
    try {
      ...
      const response = await fetch('/api/ai/generate-explanation', {
        ...
        body: JSON.stringify({ question: question.question, answer: question.answer }),
      });
      ...
      if (response.ok) {
        ...
        setExplanations((prev) => {
          const newState = [...prev];
          newState[questionIndex] = data.explanation;
          return newState;
        });
      } else {
        const data = await response.json();
        setError((prev) => {
          const newState = [...prev];
          newState[questionIndex] = data.error || 'Failed to get explanation.';
          return newState;
        });
      }
    } catch (err) {
      setError((prev) => {
        const newState = [...prev];
        newState[questionIndex] = 'An unexpected error occurred.';
        return newState;
      });
    } finally {
      setLoading((prev) => {
        ...
        <div>
          ...
          {mockQuestions.map((question, index) => (
            <div key={question.id} className="mb-6 pb-6 border-b">
              ...
              >
                {loading[index] ? 'Loading...' : 'Get AI Explanation'}
              </button>
              {error[index] && <p className="text-sm text-red-500 mt-2">{error[index]}</p>}
              {explanations[index] && (
